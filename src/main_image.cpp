...
void printFunc(int threadId) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        Lua::print(threadId);
        Lua::print(i);
        task.callMethodStatic("wait", 0, f1f);
    }
}
...
int main() {
    ...
    LuaObj shared = (LuaObj) rs.callMethod("FindFirstChild", RBXL_METHOD_HAS_RETURN_BIT, sharedStr);
    LuaObj moduleScript = (LuaObj) shared.callMethod("FindFirstChild", RBXL_METHOD_HAS_RETURN_BIT, helloStr);
    LuaObj module = (LuaObj)((LuaObj) Rbxl::getGlobal("require")).callObj(RBXL_METHOD_HAS_RETURN_BIT, moduleScript);
    ...
    LuaObj baseplate = (LuaObj) workspace.callMethod("FindFirstChild", RBXL_METHOD_HAS_RETURN_BIT, baseplateStr);
    ...
    touchedEvent.callMethod("Connect", 0, callbackHandle);
    ...
    task.callMethodStatic("wait", 0, o1f);
    //Lua::taskSpawn((void*)printFunc, 2);

    task.callMethodStatic("wait", 0, o2f);
    ...
    while (cursor < decoded.size()) {
        ...
        task.callMethodStatic("wait", 0, o0f);
        ...
        while (cursor < targetEndCursor) {
            ...
            LuaObj part;
            if (shapeType == 0) {
                part = (LuaObj) instance.callMethodStatic("new", RBXL_METHOD_HAS_RETURN_BIT, partStr);
                ...
                part = (LuaObj) instance.callMethodStatic("new", RBXL_METHOD_HAS_RETURN_BIT, wedgePartStr);
