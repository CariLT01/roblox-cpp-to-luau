...
def build_run_luau(main_address_int, main_address_str, chunk_count):
    ...
    run_lines.extend([
        ...
        "local TRACE_SIZE = 100",
        "local tracePath = table.create(TRACE_SIZE, 0)",
        "local traceIdx = 0",
        "local traceCount = 0",
        "",
        ...
        "    traceIdx = traceIdx + 1",
        "    if traceIdx > TRACE_SIZE then traceIdx = 1 end",
        "    tracePath[traceIdx] = PC",
        "    if traceCount < TRACE_SIZE then traceCount = traceCount + 1 end",
        "",
        ...
        "    local start",
        "    if traceCount < TRACE_SIZE then",
        "        start = 1",
        "    else",
        "        start = traceIdx + 1",
        "        if start > TRACE_SIZE then start = 1 end",
        "    end",
        "    local printed = 0",
        "    local i = start",
        "    while printed < traceCount do",
        '        print("Trace path: " .. tostring(printed + 1) .. " at: " .. string.format("%X", tracePath[i]))',
        "        printed = printed + 1",
        "        i = i + 1",
        "        if i > TRACE_SIZE then i = 1 end",
        "    end",
        "end",
    ])
